...
SELECT cl.language FROM countrylanguages cl
...
AND cl.isofficial = true
AND (SELECT COUNT(*) FROM countrylanguages WHERE countrycode = c.code) = 1;
...
SELECT ci.name, c.name, c.region FROM cities ci
JOIN countries c ON ci.countrycode = c.code
...
SELECT ci.name, c.name, ci.population FROM cities ci
JOIN countries c ON ci.countrycode = c.code
